...
3. Regenerate `logos/preview.html` after each iteration (most recent first)
...
- **Check small-size legibility** — Include favicon size check strip in preview. Proactively suggest fixes for thin strokes at 32px.
...
1. Identify the final iteration SVG (ask if ambiguous)
2. Create the `logos/export/` directory
3. Copy the final SVG to `logos/export/logo.svg`
4. Run the export script to generate PNGs at standard sizes: 16, 32, 48, 192, 512, 1024, 2048px
5. If no SVG-to-PNG converter found, tell the user to install one: `npm install -g @aspect-build/resvg`, Inkscape, or librsvg

## Phase 5: Repo Integration (optional)

If the user asks to commit the logo to a project repo:

1. Check for existing icon/logo files: `public/favicon.svg`, `public/favicon.ico`, `public/pwa-*.png`, `public/apple-touch-icon.png`, `assets/logo.svg`
2. Replace only files that already exist — don't add new ones the project doesn't use
3. Generate platform-specific sizes: favicon.ico (48px), apple-touch-icon.png (180px), pwa-192x192.png, pwa-512x512.png
4. Commit with message like `chore: replace logo with new [description]` and create a PR
